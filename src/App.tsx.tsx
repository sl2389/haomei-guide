...
function AudioBar({ playing, setPlaying, label = "播放音頻" }: any) {
  return (
...
function IntroPage({ setMode }: any) {
...
function MapMock({ userPos, nearest, setUserPos }: any) {
  const points = [
    [15, 72],
    [35, 52],
    [58, 33],
...
function GuidePage({ setMode }: any) {
...
function SciencePage({ setMode }: any) {
      ...
      <div className="mx-auto max-w-5xl space-y-5">
        ...
        <div className="grid gap-5 lg:grid-cols-[420px_1fr]">
          <section className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 flex min-h-[280px] flex-col items-center justify-center rounded-3xl border-2 border-dashed border-emerald-200 bg-emerald-50 p-6 text-center">
              {uploaded ? (
                <img
                  src="https://images.unsplash.com/photo-1513836279014-a89f7a76ae86?auto=format&fit=crop&w=900&q=85"
                  className="h-52 w-full rounded-2xl object-cover"
                  alt=""
                />
              ) : (
                <>
                  <Upload className="h-14 w-14 text-emerald-700" />
            ...
            <div className="mt-5 space-y-3">
              <BigButton onClick={() => setUploaded(true)} icon={Camera}>
                模擬上傳照片
              </BigButton>
              <BigButton
                onClick={() => uploaded && setIdentified(true)}
                secondary
                icon={Sparkles}
              >
                AI 初步辨識
          ...
          <section className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
            ...
            {selected && (
              <div className="mt-5 rounded-2xl bg-amber-50 p-5">
                ...
                <p className="mt-2 text-lg leading-8 text-stone-700">
                  你選擇了「{selected.name}
                  」。正式版可把照片、位置、時間與備註存入資料庫，形成好美保安林公民觀察牆。
                </p>
                <BigButton onClick={() => {}} icon={CheckCircle2}>
                  送出觀察紀錄
                </BigButton>
              </div>
            )}
